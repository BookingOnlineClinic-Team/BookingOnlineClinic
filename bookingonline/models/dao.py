...
def create_patient_user(name, username, password, email, phone=None, gender=None,bank_bin=None, bank_account_number=None, bank_account_name=None):
    user = User(name=name, username=username, password=password,email=email, phone=phone, gender=gender,role=UserRoleEnum.PATIENT)
...
def get_work_schedules_for_doctor_on_date(doctor_id, work_date):
    return (
        ...
        ).order_by(WorkSchedule.startTime).all())
...
def get_patient_profiles_by_owner(user_id):
    return PatientHealthProfile.query.filter_by(userId=user_id).order_by(PatientHealthProfile.name).all()
...
def create_patient_profile(owner, name, phone, gender=None, date_of_birth=None, address=None):
    profile = PatientHealthProfile(owner=owner,name=name,phone=phone,gender=gender,dateOfBirth=date_of_birth,address=address)
    db.session.add(profile)
    db.session.commit()
    return profile

def get_patient_profile_by_owner(profile_id, user_id):
    return PatientHealthProfile.query.filter(PatientHealthProfile.id == profile_id, PatientHealthProfile.userId == user_id).first()
...
def create_pending_payment(appointment, amount):
    payment = Payment(appointment=appointment,amount=amount,status=PaymentStatusEnum.PENDING)
...
def build_doctor_week_schedule(doctor_id,week_start):
    schedules = get_doctor_work_schedules_by_week(doctor_id,week_start)
    appointments = get_doctor_appointments_by_week(doctor_id,week_start)
    schedule_map = {}
    for schedule in schedules:
        schedule_map[(schedule.workDate,schedule.session)] = schedule
    appointment_map = {}
    for appointment in appointments:
        appointment_map.setdefault(appointment.scheduledDate,[]).append(appointment)
    days = []
    for offset in range(7):
        ...
        for appointment in appointment_map.get(current_date,[]):
        ...
        days.append({
            "date": current_date,
            "morning": schedule_map.get((current_date,WorkScheduleSessionEnum.MORNING)),
            "afternoon": schedule_map.get((current_date,WorkScheduleSessionEnum.AFTERNOON)),
            "morning_appointments":morning_appointments,
            "afternoon_appointments":afternoon_appointments})
    return days

def get_work_schedule_by_doctor_date_session(doctor_id,work_date,session):
    return (
        WorkSchedule.query
        .filter(
            WorkSchedule.doctorId == doctor_id,
...
def save_doctor_work_schedule(doctor_id,work_date,session,is_selected):
    config = get_system_config()
    schedule = get_work_schedule_by_doctor_date_session(doctor_id,work_date,session)
    ...
    if is_selected:
        ...
        if has_confirmed_appointment_in_schedule(schedule.id):
            raise ValueError("Không thể bỏ ca làm việc đã có lịch hẹn được xác nhận")
...
def is_clinic_working_day(work_date):
    ...
    weekday_code = WEEKDAY_CODE.get(work_date.weekday())
    return weekday_code in working_days

def update_doctor_week_schedule(doctor_id,week_start,selected_sessions):
    for offset in range(7):
        work_date = week_start + timedelta(days=offset)
        if not is_clinic_working_day(work_date):
            continue
        morning_key = (f"{work_date.isoformat()}_MORNING")
        afternoon_key = (f"{work_date.isoformat()}_AFTERNOON")
...
def get_doctor_appointments_by_week(doctor_id,week_start):
    week_end = week_start + timedelta(days=6)
    return (
        Appointment.query
        .filter(
